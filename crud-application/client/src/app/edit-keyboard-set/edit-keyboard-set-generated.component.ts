/*
  ...
  Modify edit-keyboard-set.component.ts instead.
*/
...
export class EditKeyboardSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getKeyboardSetById(this.parameters.Id)
    .subscribe((result: any) => {
      this.keyboardset = result;
    }, (result: any) => {
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateKeyboardSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['keyboard-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update KeyboardSet` });
    });
  }
}
