/*
  ...
  Modify add-sample-dataset-set.component.ts instead.
*/
...
export class AddSampleDatasetSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.createSampleDatasetSet(event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['sample-dataset-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to create new SampleDatasetSet!` });
    });
  }
}
