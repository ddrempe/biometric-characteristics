/*
  ...
  Modify edit-face-set.component.ts instead.
*/
...
export class EditFaceSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getFaceSetById(this.parameters.Id)
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateFaceSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['face-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update FaceSet` });
    });
  }
}
