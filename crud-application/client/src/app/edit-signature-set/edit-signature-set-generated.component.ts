/*
  ...
  Modify edit-signature-set.component.ts instead.
*/
...
export class EditSignatureSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getSignatureSetById(this.parameters.Id)
    .subscribe((result: any) => {
      this.signatureset = result;
    }, (result: any) => {
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateSignatureSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['signature-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update SignatureSet` });
    });
  }
}
