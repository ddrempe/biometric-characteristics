/*
  ...
  Modify edit-speech-set.component.ts instead.
*/
...
export class EditSpeechSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getSpeechSetById(this.parameters.Id)
    .subscribe((result: any) => {
      this.speechset = result;
    }, (result: any) => {
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateSpeechSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['speech-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update SpeechSet` });
    });
  }
}
