/*
  ...
  Modify edit-fingerprint-set.component.ts instead.
*/
...
export class EditFingerprintSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getFingerprintSetById(this.parameters.Id)
    .subscribe((result: any) => {
      this.fingerprintset = result;
    }, (result: any) => {
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateFingerprintSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['fingerprint-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update FingerprintSet` });
    });
  }
}
