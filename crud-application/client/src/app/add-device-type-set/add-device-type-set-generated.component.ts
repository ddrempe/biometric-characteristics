/*
  ...
  Modify add-device-type-set.component.ts instead.
*/
...
export class AddDeviceTypeSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.createDeviceTypeSet(event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['device-type-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to create new DeviceTypeSet!` });
    });
  }
}
