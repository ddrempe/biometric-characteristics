/*
  ...
  Modify add-sample-type-set.component.ts instead.
*/
...
export class AddSampleTypeSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.createSampleTypeSet(event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['sample-type-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to create new SampleTypeSet!` });
    });
  }
}
