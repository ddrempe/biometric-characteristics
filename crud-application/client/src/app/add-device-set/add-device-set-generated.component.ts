/*
  ...
  Modify add-device-set.component.ts instead.
*/
...
export class AddDeviceSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.createDeviceSet(event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['device-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to create new DeviceSet!` });
    });
  }
}
