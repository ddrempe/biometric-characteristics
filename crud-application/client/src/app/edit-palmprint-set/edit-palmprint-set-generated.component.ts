/*
  ...
  Modify edit-palmprint-set.component.ts instead.
*/
...
export class EditPalmprintSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getPalmprintSetById(this.parameters.Id)
    .subscribe((result: any) => {
      this.palmprintset = result;
    }, (result: any) => {
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updatePalmprintSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['palmprint-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update PalmprintSet` });
    });
  }
}
