/*
  ...
  Modify edit-gait-set.component.ts instead.
*/
...
export class EditGaitSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getGaitSetById(this.parameters.Id)
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateGaitSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['gait-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update GaitSet` });
    });
  }
}
