/*
  ...
  Modify sample-type-sets.component.ts instead.
*/
...
export class SampleTypeSetsGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getSampleTypeSets(null, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, this.grid0.allowPaging, null)
  ...
  grid0Delete(event: any) {
    this.biometricCharacteristicsModel.deleteSampleTypeSet(event.Id)
    .subscribe((result: any) => {
      this.messages.push({ severity: "success", summary: `Success`, detail: `SampleTypeSet deleted!` });
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to delete SampleTypeSet` });
    });
  }
  ...
  grid0RowSelect(event: any) {
    if (window.innerWidth >= 500) { 
      this.router.navigate([{ outlets: { popup: ['edit-sample-type-set', event.Id] } }]);
    } else {
      this.router.navigate(['edit-sample-type-set', event.Id]);
    }
  }
}
