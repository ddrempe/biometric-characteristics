/*
  ...
  Modify edit-sample-set.component.ts instead.
*/
...
export class EditSampleSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    this.biometricCharacteristicsModel.getSampleSetById(this.parameters.Id)
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.updateSampleSet(this.parameters.Id, event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['sample-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to update SampleSet` });
    });
  }
}
