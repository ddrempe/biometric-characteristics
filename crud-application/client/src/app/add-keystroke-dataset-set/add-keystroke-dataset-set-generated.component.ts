/*
  ...
  Modify add-keystroke-dataset-set.component.ts instead.
*/
...
export class AddKeystrokeDatasetSetGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  form0Submit(event: any) {
    this.biometricCharacteristicsModel.createKeystrokeDatasetSet(event)
    .subscribe((result: any) => {
      this.router.navigate([{ outlets: { popup: null } }]).then(() => this.router.navigate(['keystroke-dataset-sets']));
    }, (result: any) => {
      this.messages.push({ severity: "error", summary: `Error`, detail: `Unable to create new KeystrokeDatasetSet!` });
    });
  }
}
